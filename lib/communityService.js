 ...
 */
export async function createPost({ content, type, photoFiles = [], userIsFeatured = false, isAdminPost = false, userBadges = [] }) {
    ...
    const docRef = await addDoc(collection(db, 'posts'), {
        ...
        imageUrl,
        userId: user.uid,
        userName: isAdminPost ? 'Admin' : (user.displayName || 'Kullanıcı'),
        userPhoto: isAdminPost ? '/admin-avatar.png' : (user.photoURL || null),
        userBadges: isAdminPost ? ['admin', ...userBadges] : userBadges,
        commentCount: 0,
        likeCount: 0,
        views: 0,
        createdAt: serverTimestamp(),
        userIsFeatured: userIsFeatured || isAdminPost,
    });
    return docRef.id;
}
 ...
 */
export async function createComment({ postId, content, isAdminReply = false, userBadges = [] }) {
    const user = auth.currentUser;
    if (!user) throw new Error("Giriş yapmanız gerekiyor.");
    ...
    const docRef = await addDoc(collection(db, 'comments'), {
        ...
        content,
        userId: user.uid,
        userName: isAdminReply ? 'Admin' : (user.displayName || 'Kullanıcı'),
        userPhoto: isAdminReply ? '/admin-avatar.png' : (user.photoURL || null),
        userBadges: isAdminReply ? ['admin', ...userBadges] : userBadges,
...
/**
 * Specialty: Create an AI comment on a post for PatiAI
 */
export async function createAiComment({ postId, content }) {
    const docRef = await addDoc(collection(db, 'comments'), {
        postId,
        content,
        userId: 'patiai_bot_global',
        userName: 'PatiAI',
        userPhoto: null,
        userBadges: ['ai-vet', 'specialist'],
        isAi: true,
        createdAt: serverTimestamp(),
    });
