 ...
 */
export async function createPost({ content, type, photoFiles = [], userIsFeatured = false, isAdminPost = false }) {
 ...
 */
export async function createComment({ postId, content, isAdminReply = false }) {
    const user = auth.currentUser;
    if (!user) throw new Error("Giriş yapmanız gerekiyor.");
