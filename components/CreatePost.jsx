...
export default function CreatePost() {
    ...
    return (
        <div className={`bg-white border rounded-2xl shadow-sm p-4 mb-6 transition-all focus-within:ring-1 ${isAdminPost ? 'border-red-300 ring-red-100 shadow-md' : 'border-slate-200 focus-within:border-indigo-300 focus-within:shadow-md'}`}>
            <form onSubmit={handleSubmit}>
                {isAdmin && (
                    <div className="mb-3 flex justify-end">
                        <button
                            ...
                            className={`text-[10px] font-black px-2.5 py-1 rounded-lg border transition-all flex items-center gap-1.5 ${isAdminPost ? 'bg-red-50 text-red-700 border-red-200 shadow-sm' : 'bg-slate-50 text-slate-400 border-slate-200'}`}
                        >
                            <div className={`w-1.5 h-1.5 rounded-full ${isAdminPost ? 'bg-red-500 animate-pulse' : 'bg-slate-300'}`}></div>
                            {isAdminPost ? 'ADMIN OLARAK PAYLAŞILIYOR' : 'ADMIN OLARAK PAYLAŞ'}
                        </button>
                    </div>
                )}
                <div className="flex gap-3">
                    <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center overflow-hidden border shadow-sm ${isAdminPost ? 'border-red-200 bg-red-600' : 'bg-slate-100 border-slate-200'}`}>
                        {isAdminPost ? (
                    ...
                    <div className="flex-1">
                        <textarea
                            ...
                            placeholder={isAdminPost ? "Admin olarak önemli bir duyuru veya yanıt paylaşın..." : "Aklınızda ne var? Soru sorun veya hizmet isteyin..."}
                            className={`w-full bg-transparent border-0 border-b border-transparent placeholder:text-slate-400 focus:ring-0 resize-none outline-none py-2 font-medium min-h-[60px] text-[15px] sm:text-base transition-colors ${isAdminPost ? 'text-red-900 border-red-100 placeholder:text-red-300' : 'text-slate-800 focus:border-indigo-500'}`}
                            maxLength={500}
                        />
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between mt-3 ml-13 pt-3 border-t border-slate-100">
                    ...
                    <button
                        ...
                        className={`px-5 py-2 rounded-full font-bold text-sm transition-all shadow-sm flex items-center justify-center gap-2 ${isAdminPost ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-zinc-900 text-white hover:bg-zinc-800'}`}
                    >
                        {loading && (
                        ...
                        {loading ? '...' : (isAdminPost ? 'Admin Olarak Paylaş' : 'Paylaş')}
                    </button>
                </div>
            </form>
        </div>
